...
let profile = !!localStorage.getItem("myBarkUsername") ? `<a href="/signout?code=${encodeURIComponent(actionsecret)}" class="right">Sign out</a><a href="/profile/${localStorage.getItem("myBarkUsername")}" class="right">${localStorage.getItem("myBarkUsername")}</a></div>` : '<a href="login" class="right">Login</a><a href="signup" class="right">Join bark</a></div>'
